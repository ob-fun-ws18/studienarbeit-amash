...
module AMASH.MongoDB.Connection where
...
openConnection_ :: IO Pipe -- ^ The pipe used to communicate with the database.
openConnection_ = do
    ip <- Config.getIP
    port <- Config.getPort
...
authenticate_ :: Pipe    -- ^ The pipe used to communicate with the database.
              -> IO Bool -- ^ Whether the authentication was successful.
authenticate_ pipe = do
    user <- Config.getUser
